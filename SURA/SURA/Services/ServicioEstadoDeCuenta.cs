...
{
    class ServicioEstadoDeCuenta
    {
        //No sirve con restsharp
        ...
        public async Task DescargarEstadoDeCuenta(string miPoliza, CancellationToken ct)
        {
            try
            {
                var client = CreateClient();

                var response = await client.GetAsync($"api/EstadoCuenta?" +
                    $"identification={App.modeloDashboard.Identificacion}" +
                    $"&poliza={miPoliza}", ct);
