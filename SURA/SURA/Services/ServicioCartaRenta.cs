...
namespace SURA.Services
{
    class ServicioCartaRenta
    {
        ...
        public async Task DescargarCartaRenta(CancellationToken ct)
        {
            try
            {
                var client = CreateClient();

                var response = await client.GetAsync($"/api/CartaRenta?" +
                    $"&identification={App.modeloDashboard.Identificacion}", ct);

                if (response.IsSuccessStatusCode)
                {
                    String contenido = await response.Content.ReadAsStringAsync();
                    contenido = contenido.Replace("\"", "");
                    var sPDFDecoded = System.Convert.FromBase64String(contenido);
        ...
        private async System.Threading.Tasks.Task SolicitarPermiso()
        {
            ...
            {
                await App.Current.MainPage.DisplayAlert("SURA", "Permiso Carta de Renta: " + ex.Message, "Ok");
            }
